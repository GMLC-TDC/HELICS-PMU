...
inline void replaceIfMember(const Json::Value &element, const std::string &key, std::int16_t &sval)
{
    if (element.isMember(key))
    {
        sval = static_cast<std::int16_t>(element[key].asInt());
    }
}
...
inline void replaceIfMember(const Json::Value &element, const std::string &key, float &sval)
{
    if (element.isMember(key))
    {
        sval = element[key].asFloat();
    }
}
